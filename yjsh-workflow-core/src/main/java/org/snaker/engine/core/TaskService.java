...
public class TaskService extends AccessService implements ITaskService {
	...
	public Task complete(String taskId, String operator, Map<String, Object> args) {
		Task task = snakerDb.getTask(taskId);//access().getTask(taskId);
		...
		if(null ==history.getHistActors() || history.getHistActors().isEmpty())/*getActorIds() == null)*/ {
			//access().getTaskActorsByTaskId(task.getId());
			List<HistoryTaskActor> hactorSet = new ArrayList<>();
			for(TaskActor tactor : task.getTaskActors()){
				hactorSet.add(new HistoryTaskActor(tactor.getActorId(), history.getId()));
			}
			...
			*/
			history.setHistActors(hactorSet);
		}
		snakerDb.saveHistory(history);//access().saveHistory(history);
		snakerDb.deleteTask(task);//access().deleteTask(task);
	...
	public void updateTask(Task task) {
		snakerDb.updateTask(task);//access().updateTask(task);
	}
	...
	public HistoryTask history(Execution execution, CustomModel model) {
		...
		snakerDb.saveHistory(historyTask);//access().saveHistory(historyTask);
		return historyTask;
	}
    ...
    public Task resume(String taskId, String operator) {
        HistoryTask histTask = snakerDb.getHistTask(taskId);//access().getHistTask(taskId);
        ...
        if(isAllowed) {
            ...
            task.setTaskActors(assignTask(task, task.getOperator()));
            snakerDb.saveTask(task);//access().saveTask(task);
            return task;
        } else {
	...
	public void addTaskActor(String taskId, Integer performType, String... actors) {
		...
		switch(performType) {
		case 0:
			task.setTaskActors(assignTask(task, actors));//assignTask(task.getId(), actors);
		...
		case 1:
			try {
				for(String actor : actors) {
					...
					task.setTaskActors(assignTask(task, actors));
					snakerDb.saveTask(newTask);//access().saveTask(newTask);
	...
	public void removeTaskActor(String taskId, String... actors) {
		...
		if(task.isMajor()) {
			snakerDb.removeTaskActor(taskId, actors);//access().removeTaskActor(task.getId(), actors);
			Map<String, Object> taskData = task.getVariableMap();
	...
	public Task withdrawTask(String taskId, String operator) {
		HistoryTask hist = snakerDb.getHistTask(taskId);//access().getHistTask(taskId);
		AssertHelper.notNull(hist, "指定的历史任务[id=" + taskId + "]不存在");
		List<Task> tasks;
		if(hist.isPerformAny()) {
			tasks = snakerDb.getNextActiveTasks(hist.getId());//access().getNextActiveTasks(hist.getId());
		} else {
			tasks = snakerDb.getNextActiveTasks(hist.getOrderId(), hist.getTaskName(), hist.getParentTaskId());
		}
	...
	public Task rejectTask(ProcessModel model, Task currentTask) {
		...
		HistoryTask history =snakerDb.getHistTask(parentTaskId);
		...
		//task.setTaskActors(assignTask(task, task.getOperator()));
		snakerDb.saveTask(task);
		assignTask(task, task.getOperator());
		return task;
	}
	 ...
	 */
	private List<TaskActor> assignTask(Task task, String... actorIds) {
		if(actorIds == null || actorIds.length == 0) return null;
		List<TaskActor> set = new ArrayList<TaskActor>();
		for(String actorId : actorIds) {
			...
			set.add(taskActor);
			snakerDb.saveTaskActor(taskActor);
		}
		return set;
	}
	...
	public List<Task> createNewTask(String taskId, int taskType, String... actors) {
		Task task = snakerDb.getTask(taskId);
		AssertHelper.notNull(task, "指定的任务[id=" + taskId + "]不存在");
    ...
    public TaskModel getTaskModel(String taskId) {
        ...
        Order order = snakerDb.getOrder(task.getOrderId());
        AssertHelper.notNull(order);
        Process process = ServiceContext.getEngine().process().getProcessById(order.getProcess().getId());
	...
	private String[] getTaskActors(TaskModel model, Execution execution) {
		...
		if(StringHelper.isNotEmpty(model.getAssignee())) {
			assigneeObject = execution.getArgs().get(model.getName()+"Id");
	...
	public boolean isAllowed(Task task, String operator) {
		if(StringHelper.isNotEmpty(operator)) {
			...
			}else{//TODO：新修改为，没有操作者就是所有人都可以操作
				return true;
			}
		}
		List<TaskActor> actors = snakerDb.getTaskActorsByTaskId(task.getId());
