...
public class SurrogateInterceptor implements SnakerInterceptor {
	public void intercept(Execution execution) {
		...
		for(Task task : execution.getTasks()) {
			if(null == task.getTaskActors() || task.getTaskActors().isEmpty()) continue;
			for(TaskActor actor : task.getTaskActors()) {
				if(StringHelper.isEmpty(actor.getActorId())) continue;
				String agent = engine.manager().getSurrogate(actor.getActorId(), execution.getProcess().getName());
